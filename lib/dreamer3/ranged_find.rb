   ...
   module ClassMethods
     ...
     def count_with_when(*args)
       ...
       if (range=options.delete(:when))
         when_field=options.delete(:when_field) || :created_at
     ...
     #
     def with_dated_scope(field, time_range)
     ...
     DATE_RANGES={
       ...
       # yearly
       :last_year => [ Time.zone.now.last_year.beginning_of_year, 1.year ],
       :this_year => [ Time.zone.now.beginning_of_year, 1.year ],
       :next_year => [ Time.zone.now.next_year.beginning_of_year, 1.year ],
       # monthly
       :last_month => [ Time.zone.now.last_month.beginning_of_month, 1.month ],
     ...
     def date_range_conditions(field, time_range)
       ...
       if time_range.is_a?(Range)
         ...
         conditions << range.last.is_a?(Fixnum) ? range.first + range.last : range.last
       else
         case time_range
